    ...
    public static void main(String[] args) {
        ...
        try (Connection conn = DriverManager.getConnection(URL, USER, PASSWORD);
            ...
            stmt.executeUpdate(
                    ...
                    + "UNIQUE KEY unique_dev_jour (Developpeurs, Jour)"
                    + ")"
            );
            ...
            // Statistique 4 : total pour un développeur (PreparedStatement + Scanner)
            System.out.println("\n--- Total scripts pour un développeur (PreparedStatement) ---");

            Scanner scanner = new Scanner(System.in);
            System.out.print("Entrez le nom du développeur : ");
            String devRecherche = scanner.nextLine().trim().toUpperCase();
            ...
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ...
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next() && rs.getObject("TotalDev") != null) {
                        int totalDev = rs.getInt("TotalDev");
                        System.out.println("Total pour " + devRecherche + " : " + totalDev);
                    } else {
                        System.out.println("Aucun développeur trouvé sous le nom : " + devRecherche);
                    }
                }
            }
